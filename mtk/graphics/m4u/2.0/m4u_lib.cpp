				  ...
				  unsigned int *pMva)
{
    ...
    if(mFileDescriptor<0 )
    {
        M4UMSG("m4u_alloc_mva failed fd=%d", mFileDescriptor);
        m4u_dump_backtrace();
        return -1;
    }
    if(ioctl(mFileDescriptor, MTK_M4U_T_ALLOC_MVA, &m4u_module) < 0)
    {
        M4UMSG(" ioctl MTK_M4U_T_ALLOC_MVA fail! %d, %s", errno, strerror(errno));
        m4u_dump_backtrace();
        *pMva = 0;
	M4UMSG("ioctl MTK_M4U_T_ALLOC_MVA fail:va = %lx,size=%x,port=%d", va, size, port);
                            ...
                            unsigned int mva)
{
    ...
    {
        M4UMSG(" ioctl MTK_M4U_T_DEALLOC_MVA fail! %d, %s", errno, strerror(errno));
        m4u_dump_backtrace();
		M4UMSG("ioctl MTK_M4U_T_DEALLOC_MVA fail:va = 0x%lx,size = 0x%x,mva = 0x%x,port = %d", va, size, mva, port);
